...
import type { DepositVerification } from './types.js'
 ...
 */
export async function verifyDeposit(txHash: Hash): Promise<DepositVerification> {
  ...
  try {
    ...
    const transferLogs = receipt.logs.filter(
      (log) => log.address.toLowerCase() === PATH_USD.toLowerCase(),
    )
...
export async function getPathUSDBalance(
  address: Address,
): Promise<{ balance: bigint; balanceUSD: string }> {
  const client = createTempoPublicClient()
