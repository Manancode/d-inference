...
///     `Unmanaged.passRetained` — caller must eventually call `dginf_enclave_free()`
///   - Functions returning `UnsafeMutablePointer<CChar>?` (strings) use `strdup`
///     — caller must eventually call `dginf_enclave_free_string()`
///   - NULL return values indicate failure (Secure Enclave unavailable, invalid
///     data, signing error, etc.)
///
/// The corresponding C header would be in include/dginf_enclave.h (not yet
...
/// freed with `dginf_enclave_free()` when no longer needed.
/// Returns NULL if the Secure Enclave is unavailable or key creation fails.
@_cdecl("dginf_enclave_create")
public func dginf_enclave_create() -> UnsafeMutableRawPointer? {
...
/// `dginf_enclave_data_representation()` call. It only works on the
/// same device that generated the original key.
/// Returns NULL on failure (wrong device, corrupted data, etc.).
@_cdecl("dginf_enclave_load")
public func dginf_enclave_load(
...
@_cdecl("dginf_enclave_free")
public func dginf_enclave_free(_ ptr: UnsafeMutableRawPointer) {
    Unmanaged<AnyObject>.fromOpaque(ptr).release()
}

/// Check if the Secure Enclave is available on this device.
/// Returns 1 if available, 0 if not.
@_cdecl("dginf_enclave_is_available")
public func dginf_enclave_is_available() -> Int32 {
...
@_cdecl("dginf_enclave_public_key_base64")
public func dginf_enclave_public_key_base64(
...
/// It can be saved to disk and later passed to `dginf_enclave_load()` to
...
@_cdecl("dginf_enclave_data_representation")
public func dginf_enclave_data_representation(
    _ ptr: UnsafeRawPointer,
...
/// Caller must free with `dginf_enclave_free_string()`.
/// Returns NULL on failure (e.g., Secure Enclave unavailable, biometric denied).
@_cdecl("dginf_enclave_sign")
public func dginf_enclave_sign(
    _ ptr: UnsafeRawPointer,
    _ dataPtr: UnsafePointer<UInt8>,
    _ dataLen: Int
) -> UnsafeMutablePointer<CChar>? {
...
@_cdecl("dginf_enclave_verify")
public func dginf_enclave_verify(
...
/// `dginf_enclave_create_attestation_with_key` with no encryption key.
///
/// Returns the signed attestation as a pretty-printed JSON C string.
/// Caller must free with `dginf_enclave_free_string()`.
/// Returns NULL on failure.
@_cdecl("dginf_enclave_create_attestation")
public func dginf_enclave_create_attestation(
    _ ptr: UnsafeRawPointer
) -> UnsafeMutablePointer<CChar>? {
    return dginf_enclave_create_attestation_with_key(ptr, nil)
}
...
/// Caller must free with `dginf_enclave_free_string()`.
/// Returns NULL on failure.
@_cdecl("dginf_enclave_create_attestation_with_key")
public func dginf_enclave_create_attestation_with_key(
    _ ptr: UnsafeRawPointer,
...
/// Free a C string returned by any `dginf_enclave_*` function.
///
...
@_cdecl("dginf_enclave_free_string")
public func dginf_enclave_free_string(_ ptr: UnsafeMutablePointer<CChar>?) {
    free(ptr)
}
